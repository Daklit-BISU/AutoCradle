...
function coroutineMain()
    ...
    while true do
        ...
        while enabled do
            ...
            while enabled do
                current_position = sim.getJointPosition(cradle)
                if math.abs(current_position - position) < 0.01 then
                    -- The joint has reached its target position within the tolerance of 0.01.
                    break
                end
                sim.wait(0.01)
            end
           ...
           while enabled do
                current_position = sim.getJointPosition(cradle)
                if math.abs(current_position - 0) < 0.01 then
                    -- The joint has reached its target position within the tolerance of 0.01.
                    break
                end
                sim.wait(0.01)
